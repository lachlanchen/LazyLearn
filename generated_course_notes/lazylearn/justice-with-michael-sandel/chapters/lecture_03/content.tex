...
\begin{align}
\text{Do not use the patient} &\Longrightarrow \text{avoid bad long-run consequences}, \\
\text{Do not use the patient} &\Longrightarrow \text{respect the person as an individual}.
\end{align}
...
\begin{equation}
\text{SelfOwnership}(p)\Longrightarrow \text{the state has no title to the labor of }p.
\end{equation}
